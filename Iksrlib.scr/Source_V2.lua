...
function OrionLib:MakeWindow(WindowConfig)
	...
	WindowConfig.Size = WindowConfig.Size or UDim2.new(0, 615, 0, 344) -- ここでデフォルトサイズを設定
	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddDropdown(DropdownConfig)
				...
				DropdownConfig.Option = DropdownConfig.Option or {}
				...
				if not Dropdown.Option[Dropdown.Value] then
					Dropdown.Value = next(Dropdown.Option) or "..."
				end

				local DropdownList = MakeElement("List")

				-- Divider（仕切り）をリストの先頭に追加
				local DividerFrame = AddThemeObject(SetProps(MakeElement("Frame", Color3.fromRGB(60, 60, 60)), {
					Size = UDim2.new(1, 0, 0, 1),
				...
				-- タイトル仕切り
				local TitleFrame = AddThemeObject(SetProps(MakeElement("Frame"), {
					Size = UDim2.new(1, 0, 0, 32),
					BackgroundTransparency = 1,
					Parent = ItemParent
				}), "Second")
				local TitleLabel = AddThemeObject(SetProps(MakeElement("Label", DropdownConfig.Name, 18), {
					Size = UDim2.new(1, -12, 1, 0),
					Position = UDim2.new(0, 12, 0, 0),
					Font = Enum.Font.GothamBold,
					Name = "Title"
				...
				-- 選択中の内容表示（初期は非表示）
				local SelectedContentLabel = AddThemeObject(SetProps(MakeElement("Label", "", 14), {
				...
				local function AddOptions(OptionTable)
					for key, Option in pairs(OptionTable) do
						local OptionBtn = AddThemeObject(SetProps(SetChildren(MakeElement("Button", Color3.fromRGB(40, 40, 40)), {
							MakeElement("Corner", 0, 8),
							-- 左側Image
							AddThemeObject(SetProps(MakeElement("Image", Option.Image or ""), {
								Size = UDim2.new(0, 32, 0, 32),
								Position = UDim2.new(0, 4, 0.5, -16),
								Name = "Icon"
							}), "TextDark"),
							-- タイトル
							AddThemeObject(SetProps(MakeElement("Label", Option.Title or key, 16, 0.2), {
							...
							-- 下部説明
							AddThemeObject(SetProps(MakeElement("Label", Option.Contents or Option.ContentS or "", 13, 0.4), {
						...
						Dropdown.Buttons[key] = OptionBtn
					end
				end	

				function Dropdown:Refresh(Config, Delete)
					-- 必ず古い項目を削除
					...
					if Config and Config.Option then
						Dropdown.Option = Config.Option
					end
					AddOptions(Dropdown.Option)
				end  

				function Dropdown:Set(Value)
					if not Dropdown.Option[Value] then
						Dropdown.Value = next(Dropdown.Option) or "..."
						DropdownFrame.F.Selected.Text = Dropdown.Value
						SelectedContentLabel.Text = ""
						SelectedContentLabel.Visible = false
						for _, v in pairs(Dropdown.Buttons) do
					...
					local Option = Dropdown.Option[Value]
					DropdownFrame.F.Selected.Text = Option.Title or Value
					SelectedContentLabel.Text = Option.Contents or Option.ContentS or ""
					-- ドロップダウンが開いている時だけ説明文表示
					SelectedContentLabel.Visible = Dropdown.Toggled
				...
				AddConnection(Click.MouseButton1Click, function()
					...
					local count = 0
					for _ in pairs(Dropdown.Option) do count = count + 1 end
					if count > MaxElements then
						TweenService:Create(DropdownFrame,TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{Size = Dropdown.Toggled and UDim2.new(1, 0, 0, 70 + (MaxElements * 44)) or UDim2.new(1, 0, 0, 70)}):Play()
					else
						TweenService:Create(DropdownFrame,TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{Size = Dropdown.Toggled and UDim2.new(1, 0, 0, DropdownList.AbsoluteContentSize.Y + 70) or UDim2.new(1, 0, 0, 70)}):Play()
					end
					-- ドロップダウン開閉に合わせて説明文表示制御
					SelectedContentLabel.Visible = Dropdown.Toggled
				end)

				Dropdown:Refresh({Option = Dropdown.Option}, true)
